...
# --------------------------------------------------------------------------
# sql  部分交易详情数据，不含 确定label的trx_id信息
...
#--------------------------------------------------------------------------
rdd_part = ori_trx_new_part.rdd.mapPartitions(lambda x: rdd_fun_list(x))
user_act_part = spark.createDataFrame(rdd_part, col_list)
#user_action_part文件夹，记录 不含 label的交易数据信息
# --------------------------------------------------------------------------
# 从user_act_part中进一步group by cust_id，获取统计信息
user_act_part.registerTempTable("user_act_full")  # 注册临时表
user_act_part2 = spark.sql(
...
# --------------------------------------------------------------------------
# 保存最终的df part
user_act_part2.write.mode('overwrite').parquet('hdfs:///benchmark_data/fanxiqian/basic_data/user_action/user_action_part')
